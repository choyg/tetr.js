    ...
    if(garbage !== 0) {
      if(gameparams.backFire === 1){
        for(var y = 0; y < garbage; y++){
          this.rowRise(bottomRow, piece, true); // prevent drawing
        }
        ...
        arrRow[hole] = 0;
        for(var y = 0; y < garbage; y++){
          this.rowRise(arrRow, piece, true); // prevent drawing
        }
      }
    }
  }
 ...
 */
Stack.prototype.rowRise = function(arrRow, objPiece, noDraw) {
  noDraw = noDraw || false;
  var isEmpty = true;
  for(var x = 0; x < this.width; x++) {
